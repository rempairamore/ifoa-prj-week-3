...
*/
me.skills = ['SHELL', 'Python', 'Terraform'];
...
deleteOne = (stringa, booleano) => {
  if(booleano && typeof booleano == 'boolean' ) {
    return stringa.slice(1)
  } else if(!booleano && typeof booleano == 'boolean') {
...
*/
let whatDayIsIt = function() { return new Date().getDay() }
...
/* ESERCIZIO 8
  ...
  L'oggetto ritornato deve anche contenere una proprietà "values", contenente un array 
  con tutti i valori estratti dalle invocazioni di dice().
...
//Utilizzando il formato GG/MM/AAAA
function howManyDays(data) {
  let dataOggi = new Date().getTime() //me lo faccio dare in epoch
  let splittato = data.split('/')
  let epochInserito = new Date(`${splittato[2]}/${splittato[1]}/${splittato[0]}`).getTime()
  let differenza = Math.floor((dataOggi - epochInserito) / (1000 * 3600 * 24));
  return differenza;
}
...
function isTodayMyBirthday() {
  if(`${new Date().getDate()}/${(new Date().getMonth())+1}/${new Date().getFullYear()}` == '16/04/1992') {
    return true;
  } else {
    return false;
  }
...
function onlyInLastMillennium() {
  let soloVecchi = [];
  movies.forEach(elemento => {
    if(+elemento.Year < 2000) {
      soloVecchi.push(elemento)
    }
  })
  return soloVecchi;
}
...
function searchByTitle(stringa) {
  ...
  movies.forEach(elemento => {
    let titolo = elemento.Title.toLowerCase()
    let lowerStringa = stringa.toLowerCase()
    if(titolo.includes(lowerStringa)) {
      risultatoRicerca.push(elemento.Title)
    }
  })
...
function searchAndDivide(stringa) {
  ...
  movies.forEach(elemento => {
    ...
    if(titolo.includes(lowerStringa)) {
      ...
      unmatch.push(elemento);
    }
  })
  let oggetto = {
    matchArray: match,
    unmatchArray: unmatch
  }
  return oggetto;
}
...
*/
let backgroundColorLink = () => {
  let links = document.querySelectorAll('a')
  links.forEach(elemento => { elemento.style.backgroundColor = 'red' })
}
...
let addUl = function() {
  ...
  let nuovoItem = document.createElement('li');
  let nuovoItem2 = document.createElement('li');
  nuovoItem.innerText = 'prova elemento lista 1'
  nuovoItem2.innerText = 'prova elemento lista 2'
  ul.appendChild(nuovoItem)
  ul.appendChild(nuovoItem2)
}
...
*/
let addTrTest = function() {
  let allTr = document.querySelectorAll('tr')
  allTr.forEach(elemento => {
    elemento.classList = 'test'
  })
}
...
function isItPrime(numeroDaTestare) {
  if(numeroDaTestare > 1) {
    let numeridaProvare = numeroDaTestare - 1;
    let redFlag = 0;
    let nonNumeroPrimo = 0;
    for(let i = 2; i <= numeridaProvare; i++) {
      if(redFlag == 0) {
        if(numeroDaTestare % i === 0) {
          redFlag = 1
        }
      } else {
        nonNumeroPrimo = 1;
